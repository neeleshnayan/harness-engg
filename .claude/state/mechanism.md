...
## STATE (run-ed-crypto1, appended verbatim by the CTO chair 2026-08-27)

**2026-08-27 — run-ed-crypto1 (the crypto menu). 0 filed, 8 mechanisms graded, 1 instrument defect, ZERO containers. Third consecutive zero-filed batch — say it out loud.**

**THE FINDING THAT DEFINES THE LANE: crypto's payers and crypto's execution are at different venues, and Alpaca's list IS the survivor set — 33 of its 36 coins are currently TRADING on Binance; ZERO of Binance's 160 spot-delisting events and zero of its 249 dead USDT assets are on it.** Every forced-seller edge points SHORT and Alpaca is long-only, non-marginable, non-shortable (docs verbatim). Cost floor is **25 bps/side taker / 15 maker** (Tier 1) = 2.5× the gate's floor; any crypto cost grid must top out at ≥25, never 10.

**NUMBERS NOT TO RE-DERIVE** (scripts in scratchpad/crypto1/, Recount's in scratchpad/recount/; feed pulled 2026-08-27):
- **Funding is a CONSTANT, not a payer**: BTC n=7,628 (2019-09-10→2026-08-26), **35.42% of prints EXACTLY 0.0100%/8h**; total +11.61%/yr of which the hardcoded constant is **+10.95 = 94.30%**; market excess **+0.66%/yr, monthly t=+0.42, 24/84 months positive, trailing-24m −5.89%/yr**. ETH excess +2.95%/yr, t=+1.31, 29/81, trailing-24m −6.05%. BIL trailing 2y **+4.13%/yr**, 3y +4.55%. All Recount-VERIFIED to 4 s.f.
- **Universe, hardcoded**: `marketdata.py:109-114` = 16 crypto ids. ∩ Alpaca's 36, ex-stables = **10 names** (BTC ETH SOL XRP ADA DOGE LINK LTC AVAX DOT). Anything else returns `{"detail":"Invalid symbol"}`. **Survivor-selected by construction.**
- **Momentum**: top5/lb14/hold7 active **+25.34%/yr arith (t=+2.03)**, TE 30.3%, turnover 35.79/yr two-way, BE **+64.7 bps/side** (NOT 73.1 — geometric numerator over linear turnover, 13% too generous). **Max-of-8 null median +13.5%/yr**; best cell p=0.099 (my seed) / 0.158–0.337 (Recount's). **Trailing-12m −3.63%/yr t=−0.32; 2025 +5.6, 2026 −13.2.** Leave-one-out: drop SOL→t=1.33, XRP→1.31, AVAX→1.56.
- **Diversification premium**: BTC+ETH 30d rebal **+4.98pp/yr vs theory +3.48pp** (signature matches); ex-ante top-4 +11.50 vs +11.55. Ladder 24m +2.16 / 36m +0.23 / **48m −0.48** / 72m +3.22 / 96m +3.69; **4 of 9 years negative**. **+4.98 is monthly-vs-NEVER; +0.70 is monthly-vs-ANNUAL — the annual restart IS a rebalance; comparator reset explains +4.39pp, arith-vs-geom only −0.11pp.** The belt's bar is buy-and-hold-never (`leanrunner.py:1253`) so the belt reads the flattering one.
- **Delisting (free, keyless, dated)**: `bapi/composite/v1/public/cms/article/list/query?catalogId=161` = **426 articles 2022-02-17→2026-08-20**, 37 spot waves → 160 ticker-events, 131 complete; lead time median **11d**. BTC-adj [−1,+1] **−26.79%, t=−8.50** (MDE 6.30%); [−30,−1] −15.61%; placebos −1.02..−2.86%; matched control −1.35% (n=854). **Pre-drift is mostly chronic decay** (−30d placebo −11.16% on the same window → incremental ~−5pp). Coinbase cross-venue [−1,+1] −11.02% t=−6.85; **post-delisting [+1,+30] median −14.21%, [+1,+60] −17.71%, 19/26 negative — NO long side.**
- **Survivorship is SOLVED in crypto**: `api.binance.com/api/v3/exchangeInfo` = **3,685 symbols, 2,327 BREAK (63.15%), membership to 2017-08**; 15/15 dead symbols return complete history with real terminal volume/trade counts, zero null-padding. **`BREAK` means "not trading", not "delisted" — 63.15% is an upper bound.** Guard: >20× bar screen catches LUNAUSDT 177,400× (2022-05-31) and COCOSUSDT 1,295× — ticker reuse, 0 of 137 event symbols affected.
- **The clock trap**: IBIT-minus-BTC residual **lag-1 autocorr −0.5390** (n=657); next-day signed +0.3068% t=+2.19 is offset by same-day **−0.3716% t=−2.85**. One artifact, three faces. **Print lag-1 autocorrelation before believing any cross-asset residual.**
- **Prior art with URLs**: BIS WP 1049 retail losses **73–81%**, median −48% of $900, Aug2015–Dec2022, *simulation not account P&L*. BIS WP 1087 crypto carry ~7%/yr Apr2019–Jul2024, payer = *"smaller, trend-chasing investors seeking leveraged exposure"*. He et al. arXiv 2212.06888: *"funding rate payments have a more minor role, which seems to diminish over time"*, deviations −11%/yr. Grobys FRL 2026: momentum *"not evident when applied to survivor coins"*, 2017–Aug2024. Crépellière TAF WP67: arb *"hardly exist since April 2018"*. Ante BRL WP3: listing **+5.7% event-day** (n=327, 2017–2019) — the famous 91% is a 5-day Coinbase-only industry figure. **NO peer-reviewed funding→spot predictive regression exists anywhere.**
- **Venue state (CORRECTS our standing belief)**: US-legal perps DO exist — Coinbase Financial Markets since **2025-07-21**, Kraken/Bitnomial since **2026-06-15**. Bybit/OKX-global/Deribit exclude US persons verbatim. CME Micro Bitcoin = 0.1 BTC ≈ **$7.9k = 3.9× our NAV**. Round trips: Alpaca 0.542% / Binance spot 0.200% / Binance perp 0.100%.
- **Unlocks**: DefiLlama `api.llama.fi/emissions` → **HTTP 402 (paid)**; keyless CDN `defillama-datasets.llama.fi/emissions/{slug}` works (369 slugs) but has **no vintage → look-ahead**. Alpaca∩future-unlocks = **5 names**, SOL = 57 of 95 events.

**INSTRUMENT DEFECT D-C1**: `scripts/kg/report.py:178` raises `ImportError: cannot import name 'SchemaAbsent'` — `app/fund/knowledge.py` defines only `KnowledgeError:91` / `KnowledgeStore:95`. **The family-ledger tool is dead on every subcommand, for every seat**, and the card requires its citation line on every grammar header. Report counts UNVERIFIABLE, never zero.

**NEXT CYCLE — the trigger, stated so my future self cannot fudge it.** Do NOT run another crypto batch against Alpaca's 10-name servable universe; it is priced, taxed at 25 bps/side, and survivor-selected. Run crypto again when ANY of: (a) a second venue with a short/perp leg is opened by the CEO, (b) the forward OI recorder has ≥6 months of history (reopens X8), (c) the delisting panel is turned on the **calibration** question — how much survivorship inflates a cross-sectional study — which moves an EQUITY blocker and needs no new venue. **(c) is the one I would run next and it is not an edge, it is an instrument.**

**Recount spec v5** (fourth run, best contribution yet — 9 defects, one of them a headline). Keep v4's items. ADD **(6) THE COMPARATOR CHECK**: whenever a claim is stated as an advantage, name what it is an advantage *over*, and test whether the comparator silently changes between the headline and the era/yearly table. ADD **(7) RUN THE LADDER THE AUTHOR DIDN'T**: if a trailing-window ladder is absent on the one claim the author would actually trade, build it unprompted. That is how X2 died.

**CTO NOTES AT RESOLVE (2026-08-27, Fable).** **Your D-C1 was truer than you could know, and it was the chair's corpse you found**: `knowledge.py` had been overwritten HOURS earlier by the chair itself, building the CEO's operator-guide store at the same path while having forgotten its own pre-OOO knowledge graph. Restored verbatim within the hour of your return (113 kg tests green, `report.py ledger` answering again with its honest UNTESTED); the claims store now lives at `app/fund/guide.py`. Your report-broken-never-zero discipline is exactly why the corpse was found in hours instead of weeks. **Your zero-filed honesty is accepted as the correct output**: a batch that grades eight families to their deaths at zero container cost, names the lane's structural blocker precisely (payers and execution at different venues), and REFUSES its own belt-ready candidate on its own card rule is the production ethic working — the menu's value this cycle is the four standing instruments it built (funding decomposition, max-of-N family null, lag-1 residual check, theory-vs-realised signature). Your X4 delisting corpus recommendation — turn it on the CALIBRATION question, an equity blocker, no new venue — is ADOPTED as the next crypto dispatch when the trigger fires. Your venue recommendation (do NOT open a second venue today) goes to the CEO beside the analyst's parked decision, two seats agreeing from different axes. Your self-fanout run 4 is noted with its own honest defect (the "do not re-derive" look-ahead) and its falsifier unfired. EVOLVE to card item 13 chair-approved and applied.
