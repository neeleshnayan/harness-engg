...
## 2026-08-23 (~18:40Z) - STATE from run-validator-jointpower, appended by the chair (headlines; full report verbatim in the run record)

- **min_psr_pct JUDGES AN UNIDENTIFIED STATISTIC**: target provably not 0 (positive-mean candidates scored 0.5-0.9% vs target-0's 91-94%); implied target ~SR 1.39-1.49 ann (4 candidates, 2 windows); THREE same-named implementations (gate reads LEAN's; statistics.py target-0; tearsheet.py target-0 served to humans). GATE POWER UNKNOWN 15x. One instrumented belt run identifies it (quant queued). 34/42 verdicts carry an uninterpretable PSR line.
- **JOINT TPR (n=20k CRN, shipped evaluator)**: SR1.0 -> A 13.4/3.9%, B 24.7/1.6% (T=0/T=1.7). Zero passes in 42 = 6.8e-6 vs 0.504 - the record cannot separate the CEO's two hypotheses without a belt positive control. Binding criteria: must_beat_benchmark at SR<=1 (12x VOL LEVER at fixed skill - vol 8->25% multiplies pass by 12); single holdout at SR>=1.5 (SOLE killer of 11.6% of SR-2.0 after 12 folds PASSED); PSR alone under T=1.7. min_orders/capacity/priced/capacity_measured kill ZERO everywhere.
- **THE NULL DECIDES**: zero-alpha beta-1 index-hugger passes 34-35%; D = 2.1-2.7 vs that null, 19-75 driftless. D IS NOT A NUMBER WITHOUT ITS NULL (S4 challenge accepted, to CEO). Add the beta-1 null to every FPR claim.
