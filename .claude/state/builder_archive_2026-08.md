# builder — archived working state, 2026-08 (split 2026-08-28)

**This is the archive half of the 2026-08-28 split of `.claude/state/builder.md`** (janitor dispatch #2, chair-reviewed). It holds, VERBATIM and byte-for-byte, every entry from the seat's seeding on 2026-08-20 through the 2026-08-24 dispatches (D1 → HW3) — the first 1,103 lines of the pre-split file, including its original two-line header. **Original append order is preserved (oldest first, exactly as the chair appended them), NOT newest-first**: these entries cross-reference each other positionally ("supersedes the bullet above, by append not edit"; addenda that amend the entry they follow), and reordering would break those references while pretending to be verbatim. The live file is `.claude/state/builder.md`, whose STANDING LESSONS section distills the still-operative rules from this archive with citations back into it. **Nothing here is deleted and nothing here is dead: any item may be recalled to the live file on the builder's objection, unconditionally, and where a distilled line and its source here could disagree, THIS file is the authority.** Split verified by `builder_split_verify.py`.
