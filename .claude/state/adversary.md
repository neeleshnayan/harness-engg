...
## STATE (run-adversary-d41, appended verbatim by the chair 2026-08-24)

**2026-08-24 — run-adversary-d41 (builder-d41 re-review, the repair of my D38 kill; FOURTH round in the D36→D41 family). VERDICTS: SURVIVES (gate diff) / NOT SIGNABLE (register draft). Merge-ready with one new defect filed. Fourth consecutive round where the clock is the axis — and the first where I was the one holding it wrong.**

- **MY OWN NUMBER WAS THE WRONG ONE AND THE ARTIFACT CORRECTED ME — record it before anything else.** My D38 "366.3 obs/yr over 339 runs, true hurdle 1.2056" was the `n`-instead-of-`n−1` convention. `n` returns span `n−1` intervals: the correct rate is **365.25 exactly on all 339** (min=median=max) and the hurdle is **1.2039**. Reproduced from the shipped `scripts/instruments/d41/clocks.py`, which prints both conventions side by side for exactly this reason. **Standing check on any per-year rate I compute: state whether I divided by the observation count or the interval count, because a 0.14% factor error is invisible until somebody prints both rows.**
- **THE D38 KILL IS REPAIRED AND I PROVED IT BY EXECUTION, NOT BY READING.** `advd38/demo.py <d41-tree>` re-run unchanged: pass boundary at realised 1.367 against a stated demand of +1.34, target 1.2039, clock 365.25, one clock per payload. Under D38 the same probe printed 1.00/+1.11 while failing a strategy at 1.167. **The known-true-Sharpe boundary probe is now three-for-three as the decisive instrument on this family — keep it forever.**
- **NEW TOP ATTACK, and it is the one that found the only new defect: A REPAIR THAT CHANGES A GUARD'S INPUT MUST BE RE-RUN ON EVERY BRANCH THAT READS THAT INPUT, NOT JUST THE ONE UNDER REPAIR.** D41 moved the demand's guard from `and bar_clock` (always truthy, 252) to `and k` (`gate.py:1961`). On the ALPHA branch `k` comes from the dates and is right. On the PREMIA branch `k` is read only from `premia_inputs.strategy_excess.obs_per_year` inside the advantage-measurable branch (`gate.py:1757-1761`), so an unmeasurable advantage leaves `k=None` while the leg still holds the strategy series AND its dates. Measured at `premia_psr_basis="engine_reported"` over 767 stored results: **BASE 341 of 341 stated a demand; HEAD 0 of 341 do, and 291 are told "This run's dates do not yield a usable observation rate" when the dates yield exactly 365.25.** 291/291 false. **$0 reachability** (default `target_zero_module`, `gate.py:1246`; no production caller overrides criteria) — a disclosure regression, not a money defect, and I said so rather than inflating it.
- **AND THE GUARD TESTS FOR EXACTLY THIS CLASS COVER ONLY THE SIBLING BRANCH.** `test_the_three_absent_demand_sentences_are_mutually_exclusive` and `test_an_UNSOLVABLE_BAR_is_not_reported_as_unreadable_dates` both build `_alpha(...)` fixtures. **When a diff ships a test named for an invariant, enumerate the configurations the invariant claims to hold over and check the fixture list against them.**
- **A TEST THAT READS ITS EXPECTED VALUE OUT OF THE PAYLOAD UNDER TEST HAS ASSERTED SELF-CONSISTENCY.** `test_luck_engine_hurdle.py:513` changed to `kk = float(luck["obs_per_year"])` while its alpha sibling (`:190-196`) derives the clock from the dates independently. And its fixture is degenerate on the property: I measured both clocks in it at **260.77 / 260.77**. On real data the two differ — 8 of 8 stored runs carrying an excess pair read (251.24, 365.25), ratio 1.206, my clock factor a fourth time. Reported UNREACHABLE (0 stored runs have a measurable advantage), not measured.
- **THREE ATTACKS FAILED AND ONE OF THEM WAS MY OWN SIGNATURE MOVE — say it loudly.** (1) `clocks.py:119-120`'s "must read 1.000000" looked like the identity I killed in D38; it is not — at `LEAN_TRADING_DAYS_PER_YEAR` 260 it prints 0.984495, at 240 it prints 1.024695. **The arm can move; verified before asserting.** (2) The new tests do not bless the defect: replacing head `gate.py` with base produced **12 failed, 108 passed**, including the acceptance test. (3) The new bytecode guard fires on a real plant: I compiled `1.1 / math.sqrt(kk)` into `app/fund/__pycache__` and restored the source mtime to the pyc header's — `key_matches` True, guard FAILED naming `/lean_psr_target: CONSTANT source=1.0 cached=1.1`; its `--null` arm gives `agree=371 poisonous=0`.
- **MY D38 ZERO-CALLERS RESIDUAL IS CLOSED FOR THE CONSTANT AND OPEN FOR THE CLOCK.** Both register mutants (`LEAN_TRADING_DAYS_PER_YEAR 252→260`, numerator `1.0→1.1`) fail `test_the_recovered_target_MATCHES_THE_MODULE_CONSTANT_on_real_candidates` by name. **But nothing evaluates the observation RATE** — the only `365.25` assertions in the suite (`test_premia_inputs.py:721`, `:858`) run on hand-built synthetic calendar dates, not engine output. A sampling change would leave the register's 1.2039 stale with a green suite.
- **THE DIFF FOUND A LARGER DEFECT THAN THE ONE I KILLED, in the same class, and I verified it on my own fixture**: `sharpe_annualised` on the engine basis was RAW returns beside an EXCESS demand, gap **+0.363** — bigger than the 1.2039/1.00 clock error. **4 of 6 rows read PASS on a gate FAIL.** Withdrawn to `sharpe_annualised_raw` with an absence reason (`gate.py:1843-1854`). No consumer exists outside `gate.py` (grepped `app/`, `web/src`, `app/api`).
- **FROZEN SURFACES CERTIFIED BY EXECUTION**: `app/` diff is `gate.py` only; AST compare = 0 changed module constants, **`_luck_leg` the only changed function** (signature+body, docstring-stripped — the D37-fixed scanner); `judgement.review()` under head 19 entries `drifted: []`; re-judge of **767 × 2** = **0 verdict flips, 0 crashes, premia payload byte-identical (0 changed sentences, 0 changed keys)**; `required_sharpe_clock == obs_per_year` on 341/341, 0 rows with an annualised target and no clock; full suite **4405 passed, 1 skipped** (pre-existing worktree skip).
- **REGISTER DRAFT: NOT SIGNABLE, three small repairs.** (a) `draft:182` cites `scratchpad/d38probe/recover.py` for the 336-candidate inversion table — exists ONLY in an ephemeral agent temp dir, not in git; `clocks.py:40-43` defaults to the same dead path and gives no SQL to regenerate; `gate.py:1826` cites `scratchpad/d41probe/`. **The clock probe was shelved and its sibling was not.** (b) `falsified_by`/`review_trigger` omit the observation rate. (c) `draft:161-164` and `INSTRUMENTS.md:26` describe the null arm as "a synthetic series with exactly 252 observations a year" — there is no series in that arm. The proposed register TEXT itself (draft 203-270) is clean of scratchpad paths and of a hardcoded hurdle.
- **LIVE FACTS (re-measured this run)**: `fund_lean_jobs` **767** rows with a result (was 765 at D38); 339 carry a usable series+clock, all at 365.25; 8 carry a premia excess pair, all at ~251; **0 carry a measurable premia advantage**; `premia_psr_basis` default `target_zero_module`; `evaluate()` production call sites still exactly `fund.py:3604` and `factory.py:609`; the repo venv remains the only working interpreter; the `d41` worktree was left byte-clean (`git status --porcelain` empty after every plant).
- **Probes reusable, `scratchpad/advd41/`**: `consts.py` (module-constant + docstring-stripped function AST diff, two files, one command), `cmp.py` (verdict/sentence/key-level diff of two `judge2.py` dumps — reports flips, key adds/removes, per-key value-change counts, non-luck-check drift), `basetree/` (a `git archive`-materialised base `app/` for two-process re-judging). Still live and reused this run: `advd38/demo.py`, `advd37/judge2.py`, `advd37/astdiff2.py`.
- Kills on record: (full list carried; latest) builder D36 alpha luck LEVEL, builder D37 engine-target sentence, prod-gate pack v3 (P4-tested leg), builder D38 annualisation clause. **Repaired-and-verified: + D38 annualisation clause CERTIFIED CLOSED (boundary probe, 0 flips over 767×2, frozen surfaces re-certified); D38 zero-callers falsifier residual CLOSED for the constant.** New defect filed, not a kill: **D41-N1 premia-branch false-dates clause, 291/291, $0 reachability.**
- **Fitness**: a repair of my own kill certified closed by re-running the killing probe unchanged, plus an independent 767×2 re-judge; three attacks named as FAILED including one where my own signature move (identity-wearing-a-label) was refuted by execution before I filed it; my own D38 measurement corrected by the artifact and the correction recorded loudly; one new defect measured to 291/291 with its reachability stated as $0 rather than inflated; one latent defect reported UNREACHABLE rather than asserted.

**CHAIR NOTES AT RESOLVE (2026-08-24):** SURVIVES accepted; the D36..D41 stack merge-gated and merged by the chair on your explicit merge-approval (your own words: holding it leaves 767 alpha verdicts carrying a hurdle wrong by 20% in the permissive direction). Your 365.25 correction independently matches the builder's measurement — two instruments, blind to each other, landing on the same number is the strongest confirmation this family has produced. D41-N1 ticketed with your gate condition (must close before premia_psr_basis ever moves; that move is control-layer and takes the CEO). The register draft's three repairs ticketed for the chair; the draft stays unsigned exactly as you left it. Your test-hardening rec carried to the builder's memory with both your BINDS. The clock's five strikes now include one of yours — recorded here with the same loudness your verdicts demand of everyone else, which is the standard working.

---

## STATE (run-adversary-hw5-kp6, appended verbatim by the co-CTO 2026-08-26)

**2026-08-26 — run-adversary-hw5-kp6 (re-review of my own hw4 kill + the CEO exceptions desk). VERDICTS: SURVIVES / SURVIVES, plus one confirmed defect in the chair's own merge gate. Eighth kill→repair loop closed; first time I have proved a direction claim over an exhaustive input grid rather than a replay.**

- **NEW TOP ATTACK, and it is now my standard for any "this change can only loosen" claim: PROVE THE DIRECTION OVER AN INPUT GRID, NOT OVER HISTORY.** A historical replay only samples inputs that occurred. `advhw5/grid.py` enumerates 10 lineage shapes × status/note/next_actor (incl. `int`, `dict`, whitespace, case) = 5,940 cells, runs BOTH trees in two processes, and diffs three outcomes — refuse / allow / **RAISE**. Result: 0 tightenings, 0 new raises, 333 loosenings. **The RAISE column is the half a subset argument misses**: a repair that adds an inline import or a normaliser can 500 where the old code returned, and that is a tightening wearing an exception's clothes. Pair it with the structural argument (here: `changes` empty ⟹ `status` unchanged ⟹ v1 refused) — neither alone is enough.
- **SECOND: FOLD THE LOG AND DIFF IT AGAINST THE TABLE, WHENEVER A GUARD READS ONE AND THE WRITER WRITES THE OTHER.** Three lines of SQL + the shipped `redecision_lineage` gave `status 0 / note 0 / next_actor 0` mismatches over 491 rows — which converted a whole family of speculative seams into a measured zero and let me say SURVIVES fast. **Then find the OTHER writer**: `deskstore.py:462` `recommendations = EXCLUDED.recommendations` is an unconditional overwrite on run re-record, so the guard's premise is "no second writer" and there is one. 0 live divergence today; the mechanism is real.
- **A FAIL-OPEN CLAIM IS A CLAIM ABOUT ONE BRANCH, AND THE SIBLING BRANCH USUALLY INVERTS IT.** `next_actor` missing from an old event reads as a CHANGE (allow) on the `if na` branch and as UNCHANGED (refuse) on the `elif terminal` clear branch — same absence, opposite direction. **Ask, of any "it fails open", which branch, and what the same absence does on the others.**
- **THE ALIAS PLANT — my new standard test for any AST contract test.** Plant the field twice: through the scanned name (`r["x"]` → test FAILS, good) and through an alias the function already binds (`hit = r` at `deskstore.py:744`; `hit["x"]` → **224 passed**, blind). A source-scanning guard is anchored on a name, and names have aliases; `.update()` and `|=` are the same class.
- **RECONCILE TWO SEATS' CONFLICTING COUNTS BY DECOMPOSING, NEVER BY CHOOSING.** Chair 41/22/19 vs builder 36/22 looked like a contradiction; grouping the split by ticket type gave `decisionOwed {recommendation:19, ask:17}`, `executionOwed {recommendation:22}` — 19+22=41 recs, +17 asks = 58 = the live `desk_load.total`. **Two right answers over different denominators is the modal case here, and the decomposition is one line.**
- **HONEST NEGATIVES (do not re-spend)**: hw5 newly_refused genuinely 0 on both instruments; 37→20/17 freed (13 note-only, 4 note+na) reproduced to the unit; R39 7-of-8 under BOTH versions; 5286 passed/1 skipped exact; 236/237 and `run-pm-sleeve-v2#15`'s five-state walk exact; M4's retirement-with-proof genuine; planting v1's scope back fails 7 acceptance tests **by name** (the suite pins the repair, it does not bless it); kp6 905/905 exact; lineage `linkablePct 100` published beside `fencedPct 62.4` with `leads:"fenced"`; the 404 discriminator reads `response.status`, not the message string; `MONEY_LINE_USD=900` is NOT self-serving on the $915 anecdote (that row is `next_actor: ceo`, taken by rule 1 first).
- **MY OWN PROBE WAS THE TRAP THE BRIEF SAID IT WAS.** `advhw4/demo_note.py` calls the v1 signature and prints 17 against the repaired tree. I did not run the builder's `advhw4_demo_note_v11.py` either — it is THEIR model of MY probe. I wrote `advhw5/replay.py` from scratch, two arms, two trees. **Third dispatch running where the call-vs-model rule paid; the new clause is that a fixed-up copy of my probe handed to me by the author is still the author's model.**
- **LIVE FACTS**: log 1,569 events (678 `DeskRecommendationDecided` on 491 rows, max seq 1,545); desk table 571 rec rows (113 hold `next_actor`, 361 hold `note`, **0 terminal rows hold a next_actor**); 287 of 678 decision events carry a `next_actor` key, 58 non-null; live `desk_load.total 58` = 41 recs + 17 requests, `by_actor ceo 38 / unknown 3`, threshold 50, `coo_triage_due true`; **the ticket highway is entirely unmerged** — `GET /fund/tickets` 404s, no `app/fund/tickets.py`, no ticket table, so kp6 is inert until hw5's family lands; ticket fold today 719 (344 working / 375 terminal), `by_next_actor ceo 55 / chair 271 / seat 4 / nobody 386 / unknown 3`; folding tickets offline needs `PostgresEventStore` (NOT `PgStore`) + `DeskStore().runs(limit=1000)` — `app.main` will not import without Alpaca keys.
- **Probes reusable, `scratchpad/advhw5/`**: `replay.py` (two-tree historical replay of any guard, arm-selectable), **`grid.py` (the exhaustive direction prober — the general instrument for any "can only loosen" claim)**, `astdiff.py` (top-level + method AST diff, docstring-stripped, constants incl. `AnnAssign`), `basetree/` (`git archive`-materialised base `app/`). Node probes must live INSIDE the kp6 worktree — `node --experimental-strip-types` refuses a Windows absolute path as an ESM specifier.
- Kills on record: (full list carried; latest) builder D36 alpha luck LEVEL, builder D37 engine-target sentence, prod-gate pack v3 (P4-tested leg), builder D38 annualisation clause, **builder HW4 redecision scope**. Repaired-and-verified: + **HW4 scope kill CERTIFIED CLOSED** (0 newly-refused over 5,940 cells and 678 events; 17 freed; R39 intact). New defects filed, not kills: **merge-gate blindness on guard code (P2)**, hw5-R1 alias-blind AST scan, hw5-R2/R3 log-vs-table seam, kp6-R1 raw `t.terminal`.
- **Fitness**: two SURVIVES said loudly and fast on two unmerged stacks the CEO is waiting on, each with the killing question executed rather than argued; my own prior kill certified closed by re-derivation against the real layer (not by re-running my stale probe); four residuals filed with measured reachability of $0 rather than promoted to grounds; one confirmed defect found in the chair's own instrument while attacking something else.

**CO-CTO NOTES AT RESOLVE (2026-08-26, Fable OOO).** **The merge-gate finding is ACCEPTED against my own instrument, and you found it wider than I briefed it.** I asked you to check one content pattern; you found two independent holes — the pattern scores 0 on a diff that DELETES the guard call, and `app/fund/ticketguard.py` is absent from `SENSITIVE_PATHS` entirely, so a whole guard module classifies as ordinary. The consequence is recorded in the chair's own words: **every "0 sensitive" that gate has printed on `fund.py` is unproven, not clear**, and the constitution's requirement was satisfied by this dispatch rather than by the gate. Your note that your own hw4 probe was a trap — and that the builder's fixed-up copy of it is still the author's model — is the sharpest process line in the return and is carried to the chair's practice.

**Your two SURVIVES are accepted on the evidence; the merge is nevertheless held for the CEO's click, and that is not a hedge against your verdict.** The diff loosens a refusal control 37→20 and touches a guard module: Delegation v2 floor 3 and the co-CTO Tier-3 list both reserve that to a human, and your blind pass satisfies the first half of "adversary blind first, CEO's click always" — not the second.
...
## STATE (run-adversary-night2, appended verbatim by the CTO chair 2026-08-28)

**2026-08-27 (~00:30Z) — run-adversary-night2 (5 artifacts, blind). VERDICTS: KILL (autopolicy v5 draft) / SURVIVES (ENG2 fence) / SURVIVES (KP9 tone partition) / KILL-as-merged (routing v2) / CANNOT TELL (HYG bridge). Two SURVIVES said loudly with the killing questions executed; one CANNOT TELL because the thing has not fired.**

- **NEW TOP ATTACK, generalises to every envelope this fund will ever ship: ASK WHAT THE POLICY CAN SEE AT THE MOMENT IT DECIDES, NOT WHAT IT CHECKS.** v5 checks 23 things and every exposure bound reads the FILLED book; an order it approved 40 ms ago is invisible to all of them. Two orders each certified "14.90% against a 20.00% ceiling" build 29.80% in one name (`advn2/p4_bestcase.py`), five build 74.5% (`p3_inflight.py`). **And the obvious repair is forbidden by the envelope's own check 14** (`book == venue within 1e-6`; the broker cannot hold an unfilled order) — I attacked my own kill's weakest premise and it survived. The human's pending list is currently the only in-flight check in the fund; the pre-trade `RiskGate` is fill-based too (`risk.py:126`).
- **SECOND: A "WAY OUT" CHECK THAT NEVER READS DIRECTION.** v5 admits sells that cross zero (its own `post_fill_exposure` docstring says so) and `exitrule.enforce` raises `Side.SELL, qty=abs(qty)` — **the only `Side.` reference in the module** (`exitrule.py:326`). On a short the certified exit doubles the position. `mode.py:426-428` already registers `exit_sign_fixed` — *"Non-negotiable before anything can be short."* Money $0 today: live account `shorting_enabled: false`.
- **THE EXHAUSTIVE DIRECTION GRID PAID IN BOTH DIRECTIONS AGAIN** (`p5_grid.py`, 470 cells; `p6_venue.py`, 30 cells; `p8_dir.py`, 168 cells). It certified v5's venue leg airtight (only `VenueKind.ALPACA_PAPER.value` + `real_money is False` passes; the enum object itself refuses — `str()` of a `(str, Enum)` is `'VenueKind.ALPACA_PAPER'` on py3.11) and certified the eng3 anchor's "fences strictly fewer" claim at **0 regressions**. It also found the 17 RAISE cells and the 4 range-check holes. **Run it before writing a word about any fail-closed claim.**
- **MUTATION IS THE ONLY WAY TO GRADE A TEST'S PIN.** `unclassified` warn→quiet: **141/141 pass, blind**; the control (`fence-blind-*` warn→quiet): 1 fail. Copy the whole directory to a sibling under the repo (`studio/__advmut/`) so relative imports resolve, mutate there, `node --test --experimental-strip-types`. Never edit the committed file.
- **A ROUTER THAT MOVES MUST BE RE-RUN THROUGH EVERY BRANCH THAT READS IT — my D41 finding, now cross-repo.** `desk.OPEN_REQUEST_ACTOR` ceo→chair; `askStage` deferred correctly; `decisionList.ts:205` (`stage === "awaiting_ceo"`) and `deskLanes.ts:306` (`status === "approved"`) did not. Proved through the real modules against the live payload: **13 open asks, 0 in all five lanes, 0 in the decision list.** And fixing either predicate naively lights `deskLanes.ts:321` — 13 never-approved rows rendering "approved — the approval event recorded no actor".
- **MY OWN NEAR-MISS, TENTH IN TWELVE: I built a page-vs-spine drift kill on routing v2 and EXECUTION REFUTED IT.** `askStage` reads `next_actor_resolved` from the spine, so `countCheck` returns `null` — the two folds agree. I only found the real defect (disappearance) because I ran the probe instead of filing the argument. **Second refutation the same run:** I predicted the HYG container had restarted (`docker ps` "Up 12 minutes" vs a session row I misread as a day old) — `docker inspect` gave `RestartCount 0` and `StartedAt` matching the registry row to the millisecond. Both refutations cost minutes; both would have been wrong verdicts.
- **BEFORE ATTACKING A DRAFT WITH NO CALLER, PIN ITS CONTRACT FROM THE SIBLING THAT SHIPS.** v5's gatherer does not exist, so v4's `context_for` (`autopolicy.py:631` folds `ORDER_FILLED`) and `fund.py:7448` (one venue snapshot per TICK, outside the loop) are what make the in-flight ground a fact rather than a hypothetical.
- **LIVE FACTS**: spine 8090; clock `2026-08-27T00:23Z`; `krypton_fund` log **1,630 events**, `window_bound false`; NAV/equity **2005.12**, cash 584.41, **`shorting_enabled: false`**; mode `alpaca-paper`, `venue_kind alpaca_paper`, `real_money false`; **exactly 1 engine signal ever** (GLD 2026-08-16, `venue: paper`, `outcome: refused`, fenced), `counts.unclassified 0`; `sessions_known_since` (registry epoch) **2026-08-26T21:37:34.127841+00:00** — nothing raised after it can ever fence; one LEAN container `lean-live-2f3492903246` (`hyg_fast_flip_probe`, 26 min old, RestartCount 0, reattach); `desk_load.total 31 / threshold 50 / not due`, `requests_by_actor {ceo:0, chair:13}`, 131 requests (60 resolved / 56 approved / 13 open / 2 declined); `OrderApproved 37` — `neelesh-via-cto` has approved **zero orders ever**; `/fund/engine/ledger` 404s, the route is `/fund/engine`; KryptonPay `package.json`/`-lock` were already dirty before this dispatch (not mine).
- **Probes reusable, `scratchpad/advn2/`**: `p1_short.py` (v5 short-entry), `p3_inflight.py` + `p4_bestcase.py` (**the in-flight instrument — reuse on ANY envelope that bounds a post-fill quantity**), `p5_grid.py` (470-cell direction grid over a context dict — repoint the base and run), `p6_venue.py` (venue-spelling cross-product), `p7_fence.py` (real `engine_leg` + real `StrategyAttribution` over a synthetic event list), `p8_dir.py` (two-anchor fence direction grid). Node probes must sit INSIDE the KP repo and import with an explicit `.ts` suffix; `require` is unavailable under strip-types; argv cannot carry a desk payload (E2BIG) — write it to a file.
- Kills on record: (full list carried; latest) builder D36 alpha luck LEVEL, D37 engine-target sentence, prod-gate pack v3 (P4-tested), D38 annualisation clause, HW4 redecision scope, **autopolicy v5 draft (in-flight blindness + zero-crossing sells)**, **routing v2 as merged (CEO-page disappearance)**. New defects filed, not kills: **ENG2 sixth basis (`n_live==0` conflation, $0 reachable)**, **KP9 `unclassified` tone unpinned (141/141 blind)**, v5's string-compare pre-commitment, v5's unsigned mark check, v5's 17 raise cells, `open_request_actor` stale docstring.
- **Fitness**: two kills each demonstrated by running the shipped code rather than reading it, both with money stated as $0 today rather than inflated; two SURVIVES with the killing question executed (168-cell direction proof; 141-test mutation) and each carrying one measured defect the author had not named; one CANNOT TELL with the missing evidence named precisely; **two of my own hypotheses refuted by execution before filing**; nine failed attacks named across the batch.

## EVOLVE (chair-approved and applied, 2026-08-28)
...
## 2026-08-27 — STATE from run-adversary-v5r2 (blind re-review of the night2 kill), appended by the chair

**2026-08-27 — run-adversary-v5r2 (blind re-review of my own night2 kill). VERDICT: KILL, narrow — ONE CLAUSE of the in-flight contract. Both r1 grounds CERTIFIED CLOSED in the arithmetic and REOPENED by the specification. Ninth kill→repair loop; first time the repair was perfect in code and wrong in prose.**

- **NEW TOP ATTACK, and it is now my first move on any fold-with-a-contract: WHEN A DIFF FIXES A BLINDNESS BY ADDING A LEDGER, READ THE LEDGER'S DEFINITION OF ITS OWN MEMBERSHIP BEFORE READING ANY ARITHMETIC.** r2's in-flight fold is exact — 4,000 random cases against exhaustive 2^n enumeration, 0 mismatches — and its contract (module :71-82, memo §9.1 items 1-2) says the set is *"orders THIS ENVELOPE approved."* Every other committed-unfilled order is in the identical invisible state (book folds `ORDER_FILLED` only: `projections/positions.py:159`, `autopolicy.py:631`; the broker holds no unfilled order). Following the contract literally, `p3_scope.py` reproduces **both** killed scenarios at full size with all 29 checks green: 29.6% in one name against a 20% ceiling, and a reduce-only sell crossing zero while v4's own exit sell is invisible. **The code carries no approver field — the narrowing exists only in prose, so no test can catch it and no reviewer of the diff will see it.**
- **THE CARELESS IMPLEMENTATION IS CORRECT AND THE CAREFUL ONE IS WRONG.** "Status approved, no terminal event" is the natural query and the fund's own projection already has that state (`projections/orders.py:89,92`). Only an engineer who reads §9.1 gets it wrong. **Generalise: a spec that is narrower than the defect is more dangerous than no spec, because it will be followed.**
- **MEASURE THE WINDOW, DO NOT ASSERT IT.** 14 approve→fill pairs in the last 1,000 events: median 2.8s, **6 of 14 over 10s, 3 of 14 over 30s (a full `SETTLE_INTERVAL_SECONDS` tick), max 58.8s** — and all 14 were approved by `neelesh`, the channel the contract excludes. `run_autopolicy_tick` (`fund.py:7582-7625`) calls `context_fn(row)` per order in one loop, so it is a same-tick collision, not a race.
- **MUTATION GRADING IS NOW CHEAP AND I SHOULD RUN IT EVERY TIME.** A `conftest.py` that loads a mutated copy into `sys.modules["app.fund.X"]` and sets the package attribute lets me grade a suite without touching a committed file: copy the test files to a scratch dir, run pytest there with two env vars. **16/16 repair-reverting mutants KILLED by name** on r2 — the strongest pin I have graded here, and the exact opposite of D38's `wrong_v44`. Null control 416 passed. `p5_mutate.py` is the instrument.
- **SPLIT NO-OPS ON PAYLOAD IDENTITY, NOT VALUE IDENTITY** (the BIND, corrected in use). Type-comparing the mutant against the base value mis-split my first grid; comparing the whole `checks` payload gave the honest figure: **1,085 per-key cells = 999 REFUSE / 24 approve-changed / 30 approve-noop / 32 raise-guarded / 0 raise-escaped**, and hand-classification put **21 of the 24 as intended**. Report three numbers, never one.
- **A SHARED DENOMINATOR IS A COMMON-MODE FAILURE OF EVERY BOUND ABOVE IT.** All five of v5's dollar caps divide by one gathered `nav_usd`; `MAX_PLAUSIBLE_NAV_USD=1e12` permits a NAV **4.99e8×** the fund's real $2,002 and certifies 1.85e9 shares of HYG ($148bn) as "14.80% of NAV, inside." v4 has the same percentage shape but sits behind four quantity checks binding it to a position that already exists (`autopolicy.py:443,461,486,512`) — **v5 has no absolute-dollar ceiling anywhere.** Ask of any envelope: which single number, if wrong, makes every cap vacuous at once?
- **EXACT-STRING SYMBOL MATCHING IS A SECOND, QUIETER ROUTE BACK TO THE SAME KILL.** `in_flight:596` buckets by `r_sym == want_sym`; a near-miss spelling drops the row into `other_gross`, which enters only the 90% gross bound. At the r2 suite's OWN fixture allocation (40%), or when the in-flight row belongs to another strategy, the per-name bound reads 14.80% while the truth is 29.60%, and the reduce-only bound reads `0.000000` for a short of −3.7.
- **HONEST NEGATIVES (do not re-spend)**: both kills refuse on r2 / approve on r1 with the r1 arm run under r1's own contract; the four-corner bound exact over 4,000 random 2^n enumerations; the naive fix genuinely avoided (`:1291-1305` reads no flight term) and the control is not refuse-everything; 0 escaped raises over 1,260 cells; all four residuals closed with the boundary direction right on both sides (`_iso_lt` strict for the exit, `_iso_le` inclusive for the session — the correct assignment); `pending_approved` absence table entirely safe including `None`≠`[]`, one-bad-row-poisons-all, dict-is-not-a-ledger, age boundary inclusive at 30.0; v4 blob **da17f759 byte-identical across the whole draft family**; zero source importers; the memo corrects the dispatch brief's own wrong no-op count.
- **LIVE FACTS**: NAV **2002.44** (2026-08-27T07:46:10Z), venue equity 2005.61, cash 584.40, **buying_power 6316.99 (3.15× — margin)**, **`shorting_enabled: TRUE`** — **this contradicts my night2 record of `false`, and it moves kill 2's money off zero**; `venue.kind alpaca_paper` / `real_money false`; three deployed strategies, all with non-empty asset scopes; `/fund/autopolicy` 404 (eighth consecutive triage); `/fund/orders?limit=` 404 — routes are `/orders/pending|history|{id}`; `/fund/events` ignores `type=` and `since_seq=`, fold client-side; Postgres 5433 refused `postgres/postgres`, DSN not in `.env`.
- **Probes reusable**: `p3_scope.py` (ledger-scope instrument), `p5_mutate.py` (suite grader via conftest module-swap), `p6_denominator.py` (shared-denominator sweep), `p2b_split.py` (payload-identity no-op split), `p7_final.py` (shape table + 2^n property test), `p1_kills.py`/`p1b_r1control.py`, `p8/p9_symbol*.py` — PROMOTED to `scripts/instruments/advr2/` at resolve.
- Kills on record: (full list carried; latest) builder D36 alpha luck LEVEL, D37 engine-target sentence, prod-gate pack v3 (P4-tested), D38 annualisation clause, HW4 redecision scope, autopolicy v5 draft r1, routing v2 as merged, **autopolicy v5 draft r2 in-flight contract SCOPE clause**. Repaired-and-verified: + **v5 r1 in-flight blindness and zero-crossing sells CERTIFIED CLOSED in the arithmetic** (two-tree reproduction, 4,000-case property test, 16/16 mutants), **all four r1 residuals CERTIFIED CLOSED**. New defects filed, not kills: **v5r2-N1 exact-string symbol bucketing**, **v5r2-N2 shared NAV denominator with no absolute-dollar floor**, **v5r2-N3 order mark vs corroborated move as two inputs**.
- **Fitness**: one narrow kill demonstrated by executing the shipped code against its own written specification, with four of my own premises attacked and each verified before filing; ten honest negatives named, including the strongest test-pin I have graded here (16/16); the money on my own prior kill CORRECTED UPWARD by one live call after my last STATE recorded it as zero — the seat's credibility runs in both directions and that one runs against me.

**CTO note at resolve (Fable chair, 2026-08-27)**: three sharpest claims
verified (the contract clause verbatim; the exact-string bucket; the venue
shorting flip live). Verdict filed at docs/reviews/ADVERSARY_V5R2_2026-08-27.md.
**The prescribed repair was applied at resolve** (c5348515): contract
re-scoped to all committed-unfilled orders with your measured basis quoted in
the amendment, retention-window ban written into event-sources,
TestTheLedgerIsOriginBlind pinning both bounds at the incident's numbers,
memo §10 appended. N1 rides the fold-build charter (a blocker regardless);
N2 carried to the riskofficer as the dollar-floor argument; N3 stands as a
residual. The shorting_enabled flip is on the CEO's desk as his question.
Your self-correction on kill 2's money — recorded upward, against yourself —
is the seat working exactly as chartered. The two BINDS from the builder
about probe design were applied this run and visibly improved the grid; the
loop closed.


## 2026-08-27 — STATE from run-adversary-batch-p1-navalarm (blind batch: P1 + alarm design), appended by the chair

**2026-08-27 — run-adversary-batch-p1-navalarm (2 artifacts, blind). VERDICTS: SURVIVES (P1 Ethereum staking wrapper) / KILL (NAV-hole WARN alarm design, narrow). First SURVIVES this seat has certified by going to a third-party's primary filings instead of building a probe — and the two attacks the author asked for both came back empty, said loudly.**

- **NEW TOP ATTACK, and it should be my first move on any wrapper/share-class/ETF claim: THE ISSUER FILES THE QUANTITY, SO STOP ARGUING ABOUT THE PRICE.** SEC EDGAR gives matched quarter-end holdings and shares outstanding for both legs at zero cost: `data.sec.gov/submissions/CIK{10-digit}.json` → accession → Archives path, and `data.sec.gov/api/xbrl/companyfacts/CIK{cik}.json` gives InvestmentOwnedBalanceContracts / CryptoAssetNumberOfUnits / SharesOutstanding / NetAssetValuePerShare as clean series. Coin-per-share ratio growth of mini/ETHA: **+1.510 / +1.834 / +1.981 %/yr on three post-staking quarters, pre-staking control +0.030%/yr.** The whole price debate becomes a residual worth 10-20%. UA header required.
- **SECOND, and it saved me from filing a wrong Attack-2: A FILED "NAV PER SHARE" IS NOT THE PUBLISHED NAV.** ETHA's 10-Q NAV is a GAAP principal-market price struck at **11:59 p.m. ET**; Grayscale files against a 4:00 p.m. NY NAV. Comparing either to a 4 p.m. close measures the overnight move — I got a −1.08% "discount" on a $4bn ETF, smelled it, and read Note 2 instead of filing it. **When a premium/discount looks too big for the AUM, read the valuation-time sentence before the arithmetic.** The valid decomposition needs no NAV: price ratio = coin-per-share ratio × premium-spread ratio, exactly.
- **THIRD: DECLARED DISTRIBUTIONS ARE AN INSTRUMENT AND NOBODY USES THEM.** ETHE's 7 regular monthly pays = **1.660%/yr**; ETHB's two full months = **1.651%/yr**. Two sponsors, one number, no price noise. It converted the artifact's self-declared "weakest leg" into its strongest — the leg was weak only because a 116-session price comparison has **se ±3.55%/yr** and therefore no power. **Compute the se before calling a leg weak; "weak" and "zero-power" are different verdicts and only one is the author's fault.**
- **FOURTH, general: A FALSIFIER CAN BE ORPHANED BY A REGIME CHANGE AND STILL LOOK ALIVE.** Grayscale's Third A&R Trust Agreement (8-K, 2026-08-06/07) mandates converting staking rewards to cash and distributing them — F1 reads coin-per-share, which no longer carries the reward. Demonstrated: on the fee-gap-only regime, three consecutive 90-day windows read +0.061 / −0.012 / +0.041 %/yr, so F1's ">1.000" kills one of three with nothing wrong. **On any proposal touching a third-party product, check the issuer's filings for a structural change inside the measurement window — cheapest attack on the board, and 21 days was enough.**
- **FIFTH: ASK WHETHER THE FUND HOLDS THE THING BEING SWITCHED AWAY FROM.** `GET /fund/market/quotes?symbols=…` returns `held` per symbol; all three were false, held_count 0. A "switch, ~10bps, repaid in 4 weeks" was a **new long-ETH position at ~70%/yr vol**. One call re-labels the decision without touching the finding's value.
- **ON THE ALARM DESIGN — A LEVEL-TRIGGERED KEY SATURATES, AND THE DESIGN LANGUAGE HIDES IT.** riskmonitor raises only `new_keys = current − active`. The live record has 11 holes in 76 strikes, so a single `nav_record_holes` key is present today and a 12th hole emits **zero** events. "Self-clears when the record is whole again" is really "clears 21 days later" (`DEFAULT_LOOKBACK_HOURS = 21*24`) — proved by extending the live record with perfect strikes: lit at +20d, clear at +21d. **Whenever a design says an alarm self-clears, ask what physical act clears it; if the answer is a lookback window, it is a timer.**
- **AND COUNT THE STATES THE MODULE CAN RETURN AGAINST THE STATES THE DESIGN NAMES.** navgap has four (complete/holes/undetermined/unreadable); the design named two. The same 3-day hole fires WARN with `heartbeat.BUDGETS_SECONDS["nav_strike"]` present and reports `undetermined` — silent — with it absent, and goes permanently silent after `CALENDAR_LAST_DAY = 2027-12-31`. A rename in an unrelated module disables a control.
- **HONEST NEGATIVES (do not re-spend)**: P1 reproduces exactly (pre −0.1065 vs −0.107, post +1.874 vs +1.863, DiD +1.981 vs +1.969, monthly t +4.14 vs +4.22, daily t +0.97 vs +0.99, 10/11 months); ETHE calibration to 0.015pp; leave-one-month-out +1.476%/yr; premium drift 10-20% and sign-flips; liquidity a non-issue ($471 = 0.0014% of $33.8M median daily); counterparty story holds (BlackRock kept ETHA spot-only, launched ETHB separately). On B: all four briefed false-fire modes CLEAN (1-strike fresh fund, restart, 65h weekend, Christmas half-day + holiday, overnight); `unreadable` a proper distinct state; completeness() <1ms on 76 rows.
- **UNVERIFIABLE, reported absent**: the ~10bps switch cost (Yahoo quote 401; Grayscale behind a bot checkpoint 429×4; our quotes endpoint serves last price only — no bid/ask exists in this fund's reach today).
- **LIVE FACTS**: NAV $2,002.44 (10:37Z), cash $584.47, book SPY/DBC/TLT/DBA/IWM/QQQ/XLF — zero crypto, zero ETHA; liveness nav_record: 11 holes / 76 strikes, stale True (9,100.9 trading-s), compute_seconds 0.031, cached True, ttl 300; newest strike 2026-08-26T17:28:19Z; /fund/sleeves 404s; feed: ETH = Grayscale mini (526 bars from 2024-07-23), ETHA 526, ETHB **116 from 2026-03-12**, ETHE 526, all alpaca Adjustment.ALL.
- **Probes reusable — PROMOTED to `scripts/instruments/advp1/` at resolve**: `advA_facts.py` (the EDGAR wrapper-vs-wrapper instrument), `advA_analyze.py`/`advA_resid.py` (the price = accrual × premium decomposition), `advA_divs.py` (declared-distribution yield), `advA_ac.py` (lag-1/VR), `advB_p1.py` (the four-state alarm false-fire grid), `advB_p2.py` (saturation + self-clear timeline).
- Kills on record: (list carried; latest) …, autopolicy v5 r2 in-flight contract scope, **NAV-hole WARN alarm design (saturation + undetermined silence)**. Defects filed, not kills: P1-R1 orphaned falsifier, P1-R2 zero-power leg, P1-R3 no incumbent position, P1-R4 lag-1 −0.384, P1-R5 cost unverifiable, P1-R6 stale forward-yield inputs.
- **Fitness**: one SURVIVES certified fast on primary third-party disclosure with both author-named attacks executed and both returning empty — said loudly, because the seat's credibility runs in that direction too; one narrow KILL demonstrated by running the shipped alarm semantics against the live record; **one of my own attacks refuted by me before filing** (the filed-NAV premium series); seven honest negatives on B, six on A; money stated honestly: ~$9/yr at current sizing on A, $0 direct on B with the belief at stake named.

**CTO note at resolve (Fable chair, 2026-08-27)**: three mechanics verified
before filing (new_keys-only raise; the 21-day lookback constant; ETHA
held:false live). Filed at docs/reviews/ADVERSARY_P1_NAVALARM_2026-08-27.md
with the four alarm-redesign requirements recorded; probes promoted to the
shelf; b0d07e67 stays OPEN pending a redesign that answers both grounds. P1
routes to Stan TOMORROW as a NEW-position decision in the post-exit batch;
F1's replacement is Ed's, due 09-05. Your self-refuted Attack-2 (the
valuation-time trap caught before filing) and the loudly-said empty attacks
