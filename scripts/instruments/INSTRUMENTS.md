...
| `d41/clocks.py` | **Which clock the belt's stored series actually run on**, and what LEAN's per-observation PSR target is worth on it: obs/year, the hurdle, and what a level demands, over every stored result with readable dates — plus the `n` vs `n-1` convention side by side, because the two have been confused for one population. Null-tests itself with two arms (the module constant restated at the engine's own 252 convention must read 1.000000 — and moves with `LEAN_TRADING_DAYS_PER_YEAR`, 0.9845 at 260; a business-day series through the date pipeline reads 261.04/yr, hurdle 1.0178) and REFUSES on a missing dump rather than printing bands over zero rows. Dump shelved at `docs/drafts/data/d38_jobs_2026-08-24.json`; regenerate with `d41/pull_jobs.py`. Cited by the `min_psr_pct` register draft. | builder D41 |
...
| `advp2/advA_miss.py` | **The miss-profile table**: for any bound expressed as a %-of-portfolio-total, the per-name error each position may carry while reading MET (weight decides sensitivity; netting is free). Run on EVERY threshold over an aggregate. | adversary Leg B kill |
| `advp2/advA_fee.py` | **The non-target-term sweep**: set each term inside a statistic (other than the one it is named for) non-zero through the SHIPPED fold and find the breach level. Found the $9.96 fee breach of a 0.50% marking bound. | adversary Leg B kill |
| `advp2/advB_harness.py` + `p6_suite.py` | **The classifier mutant harness** (real git diffs through the shipped merge-gate legs, expectation column) and **the before-arm** (which mutants the SUITE kills, so a classifier miss is not auto-filed as a hole — 2 of 3 were). | adversary merge-gate kill |
| `advp2/advB_p3_census.py` + `p4_replay.py` | The AST-leg blind-spot census over a tree (1-of-138 finding) and the false-urgency replay of N real commits through any classifier (12% flag rate). | adversary merge-gate kill |
| `advp1/advA_facts.py` | **The wrapper-vs-wrapper instrument**: SEC XBRL companyfacts extractor — matched quarter-end holdings / shares outstanding for any two pooled vehicles; the issuer files the QUANTITY, so stop arguing about the price. | adversary P1 |
| `advp1/advA_resid.py` + `analyze.py` | **The price = accrual x premium decomposition** (asset price cancels exactly) — separates a yield from a re-rating with no NAV needed. `divs.py` = declared-distribution yield; `ac.py` = lag-1/VR sanity on paired closes. | adversary P1 |
| `advp1/advB_p1.py` + `p2.py` | **The four-state alarm false-fire grid** (for any rule over a multi-state reader) and the saturation/self-clear ageing timeline for level-triggered alarm keys. Run BEFORE building any alarm. | adversary NAV-alarm kill |
| `advr2/p3_scope.py` | **The ledger-scope instrument**: reproduces both v5 kills by following an in-flight contract's own membership clause literally, with control arms at the wider scope. Reuse on ANY fold whose contract names its own producer. | adversary v5r2 |
| `advr2/p5_mutate.py` | **Suite grader via conftest module-swap**: loads a mutated module copy into sys.modules without touching a committed file; 16-mutant table, null control. Reuse to grade whether any suite PINS its repairs. | adversary v5r2 |
| `advr2/p6_denominator.py` | **Shared-denominator sweep**: which single gathered number, if wrong, makes every percentage cap vacuous at once. Reuse on any envelope whose bounds divide by one input. | adversary v5r2 |
| `advr2/p2b_split.py` + `p7_final.py` | The 1,260-cell hostile grid with no-ops split on PAYLOAD identity (not value identity), and the shape table + 2^n exhaustive property test for corner arithmetic. | adversary v5r2 |
| `p2_reconcile_sampler.py N GAP` | Polls `GET /fund/venue/reconcile` N times at GAP-second intervals; prints the series (delta_usd, delta_pct, symbols_out_of_sync, book_nav, broker_equity). Read-only. The RTH re-measurement of the Leg B bound (due 2026-08-28 >13:30Z) runs exactly this. | validator p2bound |
| `p2_persym_dispersion.py N GAP` | Paired per-symbol mark dispersion: the fund's own pricer (`AlpacaConnector._fetch_price`, IEX latest trade) vs the broker position's `current_price`, same loop instant; rel-bps per name + qty-weighted USD sum. Read-only; null-tested (XLF 0.00 bps in 20/20 over a 140-obs domain). | validator p2bound |
