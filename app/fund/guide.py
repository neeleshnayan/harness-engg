"""THE OPERATOR GUIDE STORE — the firm's distilled claims, as queryable rows.

RENAMED FROM knowledge.py THE NIGHT IT WAS BORN, with an apology in the
commit: the chair built this from scratch at app/fund/knowledge.py, on the
CEO's operator-guide charter, having FORGOTTEN that a knowledge graph v1
already lived at that exact path - the hypothesis-and-kills graph of
docs/research/KNOWLEDGE_GRAPH_V1_2026-08-23.md, validator-repaired and
mutation-hardened - and overwrote it. Ed's crypto batch found the corpse
within hours (scripts/kg/report.py dead on every subcommand). The original
is restored verbatim; THIS module is the complementary half: knowledge.py
answers "what did we TEST and what killed it"; guide.py answers "what do we
KNOW and what is the receipt". The two share a database and must never
share a table.
...
#: The isolation guard (tests/test_knowledge_isolation.py) derives its
#: forbidden-import set by scanning app/fund for this flag.
