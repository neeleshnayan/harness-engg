...
ORPHAN_NOTE = (
    ...
    "reconciliation on a timer WHEN IT IS RUNNING AND HOLDS THE SCHEDULER "
    "LEASE — so the window in which a container can be unaccounted for is "
    "bounded by LEAN_RECONCILE_INTERVAL rather than by how long the spine has "
    "been up, on a spine whose worker is alive. What remains is that this fold reads the "
