...
Reproduce: `scripts/instruments/d41/clocks.py <tree> [jobs.json]` (the dump is
shelved at `docs/drafts/data/d38_jobs_2026-08-24.json`, regenerated by
`scripts/instruments/d41/pull_jobs.py` — one read-only `SELECT job_id, result
FROM fund_lean_jobs WHERE result IS NOT NULL`). It carries its own NULL TEST,
two arms, described exactly: the module constant restated at the engine's own
252 convention must read 1.000000 — and the arm can MOVE, which is what makes
it a measurement rather than an identity (at `LEAN_TRADING_DAYS_PER_YEAR` 260
it reads 0.984495, at 240 it reads 1.024695); and a business-day series read
through the same date pipeline yields 261.04 observations a year and a hurdle
of 1.0178. Without those arms the table above cannot distinguish "measured the
clock" from "printed a constant". It also
...
(n = 336; reproduce from the shelved dump `docs/drafts/data/
d38_jobs_2026-08-24.json` via `statistics.implied_target_sharpe` over every
row carrying `psr_pct`, `series` and `dates` — the dump itself regenerates
with `scripts/instruments/d41/pull_jobs.py`, one read-only `SELECT job_id,
result FROM fund_lean_jobs WHERE result IS NOT NULL`):
...
> AND THE CLOCK HALF, SEPARATELY (adversary D41: an entry whose reason and
> whose falsifier are about different quantities is the register's founding
> failure mode): the 1.2039 this entry leads with is a function of the belt's
> SAMPLING RATE — one observation per calendar day, measured 365.25 obs/yr,
> min = median = max over all 339 usable stored series. If the belt's emitted
> series ever moves off that sampling (business-day emission would read
> ~261/yr and a hurdle of ~1.018), the annualised figures here go stale WHILE
> THE SUITE STAYS GREEN — the suite's evaluator watches the engine's
> per-observation constant, not the rate. The rate's evaluator is
> `scripts/instruments/d41/clocks.py` re-run against a fresh pull, and it is
> a review-time act, not a suite guard, stated so nobody mistakes one for the
> other."
>
> **review_trigger:** "a LEAN image change that moves
> `algorithmConfiguration.tradingDaysPerYear` or the PSR benchmark constant; a
> market-neutral / short-capable candidate reaching the belt; or any new belt
> result whose stored series is no longer one-observation-per-calendar-day
> (`clocks.py` reading obs_per_year off 365.25 — the sampling-rate half of
> this entry, which no suite test evaluates)"
