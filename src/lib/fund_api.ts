...
export interface DeskLoad {
  ...
  /** The digest of the desk-stage CONTRACT the spine's routing was generated
   *  against (`contract/desk_stage_contract.v1.json`, checked in to both
   *  repos byte-identical and generated by `desk.py` itself).
   *
   *  This page and the spine's counter are two implementations of one
   *  question and have disagreed twice — 11 vs 6, then 1 vs 0 on an accepted
   *  row the CEO still owned. Each repo's suite now pins its own copy of the
   *  contract, which catches an edit on either side and does NOT catch this
   *  repo going stale against a regenerated spine; there is no shared build
   *  for a hermetic test to live in. So the digest travels on the wire and
   *  `contractDrift()` compares it.
   *
   *  `null` = the spine could not read its contract file, or the file failed
   *  its own digest. Absent = a spine that predates the field. Both are
   *  UNVERIFIED and neither is agreement — see `contractDrift`. */
