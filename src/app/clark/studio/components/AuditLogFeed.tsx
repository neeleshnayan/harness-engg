  ...
  return (
    <div className={`${KT.panel} p-4 space-y-3`}>
      ...
      <div className="divide-y divide-zinc-800/60 max-h-[380px] overflow-y-auto pr-1">
        {events.length === 0 ? (
          <div className="py-8 text-center text-xs text-[var(--kt-text-muted)]">No events logged yet.</div>
        ) : (
          events.map((e) => {
            const badge = EVENT_TYPE_BADGES[e.type] || {
              label: e.type,
              color: "bg-[var(--kt-inset)] text-[var(--kt-text-dim)] border-[var(--kt-border)]",
            };
            ...
            return (
              <div key={e.event_id} className="py-2.5 space-y-2">
                <div
                  className="flex items-center justify-between gap-3 text-xs cursor-pointer hover:bg-[var(--kt-surface)] p-1.5 rounded-md transition-colors"
                  onClick={() => toggleExpand(e.event_id)}
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-[var(--kt-text-muted)] text-[11px] font-mono tabular-nums">#{e.seq}</span>
                    <span className={`rounded border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${badge.color}`}>
                      {badge.label}
                    </span>
                    <span className="truncate text-[var(--kt-text-dim)] font-medium">{e.aggregate_id}</span>
                  </div>
