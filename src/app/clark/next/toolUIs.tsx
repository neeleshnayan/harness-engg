...
/**
 * Generative UI, the provenance-first way: every component here renders a
 * tool's ACTUAL return value from the spine. The model chose which question
 * to ask; it does not get to draw the answer. A tool whose result did not fit
 * the stream's size cap falls back to its one-line preview — an honest stub,
 * never an invented chart.
 *
 * Registered by mounting inside the runtime provider (makeAssistantToolUI).
 * Styled with KT tokens with dark fallbacks so the components hold up in
 * both Clark themes.
 */
...
function MiniSeries({ points, up }: { points: { x: string; y: number }[]; up: boolean }) {
  return (
...
function PreviewOnly({ preview }: { preview?: string }) {
...
export const FundNavUI = makeAssistantToolUI<Record<string, unknown>, NavResult>({
  toolName: 'fund_nav',
  render: ({ result, status }) => {
    if (status.type === 'running' || !result) return null
...
export const FundBacktestUI = makeAssistantToolUI<Record<string, unknown>, BacktestResult>({
