...
export const PriceChart: React.FC<PriceChartProps> = ({ data }) => {
    const chartData = data.map(s => ({
        ...
        price: Number(s.wethPrice ?? '0')
    })).sort((a, b) => a.timestamp - b.timestamp);

    return (
        <div className="rounded-2xl border border-zinc-700/50 bg-zinc-800/50 p-6 backdrop-blur">
            <h3 className="text-lg font-bold text-white mb-1">WETH Price</h3>
            <p className="text-xs text-zinc-400 mb-6">REAL-TIME PRICE FROM QUOTER (USDC)</p>
