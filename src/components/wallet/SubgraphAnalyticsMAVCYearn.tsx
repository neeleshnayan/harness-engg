...
interface SubgraphAnalyticsMAVCYearnProps {
  subgraphUrl?: string;
}
    ...
    return (
      <div className="mt-8 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-6 py-5 text-amber-100">
        ...
        <p className="mt-2 text-sm text-amber-50/80">
          Configure the SUBGRAPH_URL field in Firestore (quant_strategies/MAVC_YEARN) to enable analytics.
        </p>
      </div>
    );
  }
      ...
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400">
            Live MAVC Yearn vault metrics powered by The Graph subgraph.
          </p>
        </div>
        {data && (
          <button
      ...
      {!metrics && !isLoading && !isError && (
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-6 py-5 text-amber-100">
          ...
          <ul className="mt-2 text-xs text-amber-50/60 list-disc list-inside space-y-1">
            ...
            <li>The metric entity hasn't been created (id: "mavc-yearn-vault")</li>
            <li>No deposits or withdrawals have occurred</li>
          </ul>
          <div className="mt-3 pt-3 border-t border-amber-500/20">
            <p className="text-xs font-semibold text-amber-50/80 mb-1">Query Status:</p>
            <ul className="text-xs text-amber-50/60 space-y-0.5">
              <li>Deposits found: {deposits.length}</li>
              <li>Withdrawals found: {withdrawals.length}</li>
              <li>Metric entity: {metrics ? 'Found' : 'Not found'}</li>
            </ul>
          </div>
      ...
      {hasChartData && (
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-2xl border border-zinc-700/50 bg-zinc-800/50 p-6 shadow-2xl backdrop-blur">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-white">Share Mint vs Burn</h3>
              ...
              {deposits.slice(0, 6).map((entry) => (
                <li key={entry.id} className="rounded-xl bg-zinc-700/50 px-4 py-3">
                  ...
                  <div className="mt-2 flex justify-between text-sm text-white">
                    ...
                    <span>{formatShare(Number(entry.shares ?? '0'))} Shares</span>
                  </div>
                </li>
              ))}
            </ul>
