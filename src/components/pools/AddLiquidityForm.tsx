...
export default function AddLiquidityForm({
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      const result = await nettingPoolsApi.addLiquidity({
      ...
      setSuccess(`Liquidity addition submitted! Transaction ID: ${result.transaction_id}`);
      setAmountToken0('');
      setAmountToken1('');
      setTimeout(() => {
        onSuccess?.();
      ...
      setError('Add liquidity failed: ' + (err.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="text-gray-400 text-sm mb-4">
        Add liquidity to the pool by providing both tokens. This increases the pool's depth and your share of trading fees.
      </div>
      ...
      {amountToken0 && amountToken1 && parseFloat(amountToken0) > 0 && parseFloat(amountToken1) > 0 && (
        <div className="text-gray-400 text-xs text-center">
          Price ratio: 1 {token0Symbol} = {(parseFloat(amountToken1) / parseFloat(amountToken0)).toFixed(6)} {token1Symbol}
        </div>
      )}
